...
| **AI 管线提取** | Claude 驱动的 5 阶段管线：分类 → 元数据补全 → 内容蒸馏 → 标签生成 → 卡片渲染 |
...
| 阶段 | Skill | 模型 | 职责 |
|------|-------|------|------|
| 1 | **PageSense** | Haiku | 页面类型分类（8 种类型） |
| 2 | **Contextualizer** | Haiku | 元数据补全（作者 / 日期 / 语言 / 阅读时间） |
| 3 | **Distiller** | Sonnet | 内容结构化提取（按类型定制 prompt） |
...
| AI | Anthropic Claude (Haiku + Sonnet), Structured Output (`tool_use`) |
